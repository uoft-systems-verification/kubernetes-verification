...
Proof.
  ...
  set I := (∃ (i: w64) (val: interface.t) (sl': slice.t) (ptr_list': list loc) (pure_pod_list': list PurePod.t),
    ...
    "Hlist_pre" ∷ ([∗ list] ptr;pure_pod ∈ ptr_list';pure_pod_list', ∃ pod, PurePod.deepown_l ptr pod pure_pod 1) ∗
    "Hlist_post" ∷ ([∗ list] ptr;pure_pod ∈ (drop (sint.nat i) ptr_list);(drop (sint.nat i) pure_pod_list), ∃ pod, PurePod.deepown_l ptr pod pure_pod 1) ∗
  ...
  - wp_pure; first word.
    ...
    + wp_for_post.
      ...
      iFrame.
  - iAssert (⌜ ∀ key pod, pure_pod_map !! key = Some pod → abs_state !! key = Some (PureKObject.Pod pod) ⌝%I)
    as "%Hsub_map".

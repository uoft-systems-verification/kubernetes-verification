...
(** The scalar-footprint counterpart of [wp_GetName_deepown_kobject], for
    callers that share an object's metadata read-only. *)
Lemma wp_GetName_scalars_kobject i l o c m dq:
  {{{ is_pkg_init code.k8s_io.apimachinery.pkg.apis.meta.v1.pkg_id.v1 ∗
      "%Hi" ∷ ⌜ KObjectV.valid_interface i l o ⌝ ∗
      "Hmeta" ∷ ObjectMetaV.own_scalars (KObjectV.objectmeta_ptr l o) c m dq
  }}}
    #(methods i.(interface.ty) "GetName" i.(interface.v)) #()
  {{{ RET #m.(ObjectMetaV.Name');
      ObjectMetaV.own_scalars (KObjectV.objectmeta_ptr l o) c m dq
  }}}.
Proof.
  wp_start as "H". iNamed "H".
  destruct o; simpl in Hi; destruct Hi as [-> _]; wp_method_call;
    wp_pures; wp_apply (wp_GetName_scalars with "[$Hmeta]");
    iIntros "Hmeta"; iApply "HΦ"; iFrame.
Qed.

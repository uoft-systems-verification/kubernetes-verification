From New.proof.k8s_io.apimachinery.pkg.apis.meta Require Export v1_init.
From New.proof.k8s_io.kubernetes.pkg.apis Require Export core_init.
Require Export New.generatedproof.k8s_io.kubernetes.pkg.apis.apps.
From New.proof Require Import proof_prelude.


Section proof.
Context `{hG: heapGS Σ} `{!ffi_semantics _ _}.
Context {sem : go.Semantics} {package_sem : apps.Assumptions}.
Collection W := sem + package_sem.

#[global] Instance : IsPkgInit (iProp Σ) apps := define_is_pkg_init True%I.
#[global] Instance : GetIsPkgInitWf (iProp Σ) apps := build_get_is_pkg_init_wf.

End proof.

...
(* PersistentVolumeClaim.DeepCopy is generated by Kubernetes' deepcopy
   ...
   contract. *)
Lemma wp_PersistentVolumeClaim__DeepCopy claim_template_l claim_template dq :
  {{{ PersistentVolumeClaimV.deepown_l
        claim_template_l claim_template dq }}}
    claim_template_l @! (go.PointerType v1.PersistentVolumeClaim) @!
      "DeepCopy" #()
  {{{ claim_l, RET #claim_l;
      PersistentVolumeClaimV.deepown_l claim_l claim_template 1 ∗
      PersistentVolumeClaimV.deepown_l
        claim_template_l claim_template dq
  }}}.
Proof. Admitted.
...
Proof.
  ...
  wp_auto.
  wp_apply (wp_PersistentVolumeClaim__DeepCopy with "Hclaim_template").
  iIntros (claim_l) "[Hclaim Hclaim_template]".
  wp_auto.
  ...
  all:
    ...
    | _ = Some _ =>
      iAssert (StatefulSetSpecV.deepown set_spec_c
        ...
        with "[Hset_spec_Hdeepown_replicas_some
               Hset_selector_some
               Hset_spec_Hdeepown_template
               Hset_spec_Hdeepown_volumeclaimtemplates]"
        as "Hset_spec";
      [rewrite /StatefulSetSpecV.deepown Hselector_opt /=;
       iSplit;
       [iPureIntro;
        exact Hset_spec_Hdeepown_replicas_none|];
       iSplitL "Hset_spec_Hdeepown_replicas_some";
       [iExact "Hset_spec_Hdeepown_replicas_some"|];
       iSplit;
       [iPureIntro;
        exact Hset_spec_Hdeepown_selector_none|];
       iSplitL "Hset_selector_some";
       [iExact "Hset_selector_some"|];
       iFrame;
       iPureIntro;
       exact Hset_spec_Hdeepown_servicename|]
    | _ = None =>
      iAssert (StatefulSetSpecV.deepown set_spec_c
          set.(StatefulSetV.Spec') dq_set)
        with "[Hset_spec_Hdeepown_replicas_some
  ...
  all:
    iAssert (StatefulSetSpecV.deepown_l
        (StatefulSetV.spec_ptr set_l)
        set.(StatefulSetV.Spec') dq_set)
      with "[Hset_spec_field Hset_spec]" as "Hset_spec_l";
    [iExists set_spec_c; iFrame|].
  all:
    iPoseProof (StatefulSetV.deepown_l_restore
      _ _ _ Hset_l_not_null with
      "[$Hset_typemeta $Hset_objectmeta_l
  ...
  all:
    ...
    iAssert (ObjectMetaV.deepown_l
        (PodV.objectmeta_ptr pod_l)
        pod.(PodV.ObjectMeta') dq_pod)
      with "[Hpod_objectmeta_field Hpod_objectmeta]"
      as "Hpod_objectmeta_l";
    [iExists pod_meta_c; iFrame|];
    iPoseProof (PodV.deepown_l_restore
      _ _ _ Hpod_l_not_null with
      "[$Hpod_typemeta $Hpod_objectmeta_l

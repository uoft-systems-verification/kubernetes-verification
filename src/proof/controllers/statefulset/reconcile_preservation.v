...
Definition own_unreserved_pods γ (reserved pods : list PodV.t) : iProp Σ :=
  ([∗ list] pod ∈ filter (pod_key_not_reserved reserved) pods,
...
Proof.
  ...
  - match goal with
    ...
    + simpl in Hfind.
      ...
      iEval (rewrite /own_unreserved_pods Hcurrent_decomp
      ...
      iAssert (own_unreserved_pods γ (filter (pending_pod set) pods)
        ...
        with "[Hmeta_before Hupdate_Hown_meta Hmeta_after
          Hspec_before Hupdate_Hown_spec Hspec_after]" as "Hown_pods".
      { rewrite /own_unreserved_pods
    ...
    + simpl in Hfind. subst pod_l.
      ...
      iAssert (own_unreserved_pods γ (filter (pending_pod set) pods)
        ...
        as "Hown_pods".
      { unfold own_unreserved_pods, current_pods'.
...
Lemma wp_reconcileCondemnedPod_preservation γ model_l set_l pods_sl
  ...
  {{{ (pods' : list PodV.t), RET (#false, #interface.nil);
      "Hset" ∷ StatefulSetV.deepown_l set_l set dq_set ∗
      "Hpods_sl" ∷ pods_sl ↦* ptrs ∗
      "Hpods" ∷ ([∗ list] ptr;pod ∈ ptrs;local_pods,
        PodV.deepown_l ptr pod dq_pods) ∗
...
Proof.
  ...
  - pose proof (local_pods_match_stored_lookup
    ...
    iEval (rewrite /own_unreserved_pods Hpods_decomp
    ...
    iDestruct "Hstored_own" as "[Hstored_meta Hstored_spec]".
    assert (PodV.key stored_pod ∈
    ...
    + iSpecialize ("Hlocal_pod_restore" with "Hdelete_Hpod").
      ...
      { unfold stored_pod'. rewrite /PodV.key /PodV.meta_key /=
      ...
      iAssert (own_unreserved_pods γ
          ...
          (before ++ stored_pod' :: after))
        with "[Hown_before Hstored_meta' Hstored_spec Hown_after]"
        as "Hown_pods".
      { unfold own_unreserved_pods.
        rewrite (filter_replace_reserved
          (filter (pending_pod set) local_pods) before stored_pod
          stored_pod' after Hstored_unreserved (eq_sym Hstored_key')).
      ...
      { rewrite Hchildren_keys. iExact "Hdelete_Hown_children". }
      ...
      assert (NoDup (PodV.key <$> (before ++ stored_pod' :: after)))
        as Hnodup'.
      { rewrite Hchildren_keys. exact Hstored_nodup. }
      assert (filter (pending_pod set) local_pods ⊆
          before ++ stored_pod' :: after)
        as Hreserved_subset'.
      { eapply (reserved_subset_replace _ _ stored_pod stored_pod' _).
        - rewrite -Hpods_decomp. exact Hreserved_subset.
        - exact Hstored_unreserved. }
      wp_auto. iApply ("HΦ" $! (before ++ stored_pod' :: after)).
    ...
    + iSpecialize ("Hlocal_pod_restore" with "Hdelete_Hpod").
      ...
      { unfold own_unreserved_pods. rewrite list.filter_app big_sepL_app.
        iFrame. }
      assert (NoDup (PodV.key <$> (before ++ stored_pod :: after)))
        as Hdecomp_nodup.
      ...
      { eapply (reserved_subset_remove _ _ stored_pod _).
        - rewrite -Hpods_decomp. exact Hreserved_subset.
        - exact Hstored_unreserved. }
      wp_auto. iApply ("HΦ" $! (before ++ after)).
      iFrame. iPureIntro. split_and!; try done. lia.
Qed.
...
Proof.
  ...
  iIntros (continue_desired pods1 pvc_map1)
    "(Hset & Hpods_sl & Hpods & Hown_pods & Hown_pvcs &
      Hown_children & %Hpvc_wf1 & %Hdistance1 & %Hmembers1 &
      %Hnodup1 & %Hreserved_subset1 & Hdesired_result)".
  ...
  - iDestruct "Hdesired_continue" as
    ...
    wp_apply (wp_reconcileCondemnedPod_preservation γ model_l set_l
      pods_sl set ptrs pods pods1 reserved (pvc_list_of_map pvc_map1)
      pvcs dq_set dq_pods with
      "[$Hset $Hpods_sl $Hpods $Hown_pods $Hown_children]").
    { iFrame "#". iPureIntro. split_and!; done. }
    iIntros (pods2)
      "(Hset & Hpods_sl & Hpods & Hown_pods & Hown_children &
  ...
  - iDestruct "Hdesired_stop" as "%Hcontinue_desired".
    ...
    iEval (rewrite (own_pvc_map_as_list γ pvc_map1 Hpvc_wf1))
      in "Hown_pvcs".
    iApply ("HΦ" $! pods1 (pvc_list_of_map pvc_map1)).
    iFrame. iPureIntro. split_and!; done.
Qed.

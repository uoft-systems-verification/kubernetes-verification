...
Proof.
  ...
  { iFrame "#". iFrame. iPureIntro. split_and!.
    ...
    - rewrite /create_prepared_for_helper /=.
      ...
      rewrite decide_False in Hvalid_meta_create.
      1: exact Hname_nonempty.
      destruct Hvalid_meta_create as ((_ & Hname_valid) & _).
  ...
  { destruct kobj, kobj2;
      simpl in Howner_references2, Hparent_none |- *; try done.
    all: unfold obj_parent_ref, meta_parent_ref in Hparent_none |- *;
      simpl in Hparent_none |- *;
      lazymatch type of Howner_references2 with
      | _ = ?input_refs => destruct input_refs as [orefs|] eqn:Horefs
      end;
      try rewrite Horefs in Howner_references2;
      try rewrite Horefs in Hparent_none;
      try rewrite Horefs;
      rewrite Howner_references2; simpl in Hparent_none |- *; [|done];
      destruct (list_find
  ...
  { symmetry. exact Hkobj2_uid. }
  { exact Hkey_not_in_abs. }
  { unfold living_obj_parent_ref. rewrite Hdeletion_timestamp2 /= Hparent_none2. done. }
...
Lemma wp_State__PersistentVolumeClaimCreate_named_orphan γ l namespace key pvc_l pvc :
  {{{ is_pkg_init apimodel ∗
      "#Hisk" ∷ is_kubernetes γ l ∗
      "%Hvalid" ∷ ⌜ PersistentVolumeClaimV.valid_create PersistentVolumeClaimV.kind namespace pvc ⌝ ∗
      "%Hname_nonempty" ∷ ⌜ pvc.(PersistentVolumeClaimV.ObjectMeta').(ObjectMetaV.Name') ≠ ""%go ⌝ ∗
      "%Hkey_eq" ∷ ⌜ key = {|
  ...
  {{{ pvc_l' pvc' uid, RET (#pvc_l', #interface.nil);
      ...
      "%Hcreated" ∷ ⌜ PersistentVolumeClaimV.created namespace pvc pvc' ⌝ ∗

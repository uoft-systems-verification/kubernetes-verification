...
Lemma wp_State__update_au γ l kind namespace i kobj :
  ∀ Φ,
    ...
    (|={⊤, ∅}=> ∃ key uid kmeta kspec,
      ...
      "%Hvalid_update" ∷ ⌜ KObjectV.valid_update kind namespace kmeta kspec kobj ⌝ ∗
      "%Hvalid_simple_update" ∷ ⌜ ObjectMetaV.valid_simple_update kmeta (KObjectV.objectmeta kobj) ⌝ ∗
      "Hclose" ∷ (
        (∀ i' kobj',
          (⌜ KObjectV.valid kobj' ⌝ ∗
           ⌜ KObjectV.updated kobj kobj' ⌝ ∗
...
Proof.
  ...
  2: {
    iApply fupd_wp.
    iMod "Hau" as (key uid kmeta kspec) "H". iNamed "H".
    exfalso. apply Hinvalid_update_input.
    revert Hvalid_update.
    destruct kspec, kobj; rewrite /KObjectV.valid_update /=;
  ...
  2: {
    ...
    assert (ObjectMetaV.valid_update kmeta (KObjectV.objectmeta kobj) ∧
        ObjectSpecV.valid_update kspec (KObjectV.spec kobj)) as
      (Hvalid_meta_update & Hvalid_spec_update).
    { destruct kspec, kobj; rewrite /KObjectV.valid_update /= in Hvalid_update |- *;
        rewrite ?/PodV.valid_update ?/ReplicaSetV.valid_update
  ...
  { destruct old_kobj, kobj; rewrite /KObjectV.valid_update /=;
      ...
      simpl in Hvalid_meta_update_actual, Hvalid_spec_old;
      try contradiction; tauto. }
  ...
  wp_apply (wp_applyValidationAndDefaultingOnUpdate_ok _ _ _ _ _ _ _
    (KObjectV.objectmeta kobj).(ObjectMetaV.Namespace') kobj
  ...
  pose proof Hhelper_updated as (Hstatus_eq & _ & Hhelper_result_updated & _).
  ...
  pose proof (applyValidationAndDefaultingOnUpdate_updated_implies_valid
    (KObjectV.kind kobj) (KObjectV.objectmeta kobj).(ObjectMetaV.Namespace') old_kobj kobj kobj
    updated_kobj Hvalid_old_kobj Hvalid_update_actual
    Hprepared Hhelper_updated) as Hvalid_updated_kobj.
  assert (KObjectV.same_kind kobj updated_kobj ∧
      KObjectV.typemeta updated_kobj = KObjectV.typemeta kobj ∧
      ObjectSpecV.updated (KObjectV.spec kobj) (KObjectV.spec updated_kobj) ∧
    ...
    as (Hsame_kind & Htypemeta_eq & Hupdated_spec & Hname_updated &
      Hnamespace_updated & Huid_updated & Hdeletion_timestamp_updated).
  ...
  { rewrite (kobject_updated_parent_ref _ _ Hupdated_kobj).
  ...
  wp_if_destruct.
  {
    ...
    pose proof (Hhelper_result_updated kobj old_kobj Hprepared Hequiv_updated_old)
      as Hupdated_old.
    iApply fupd_wp.
    iMod "Hau" as (key0 uid kmeta kspec) "H". iNamed "H".
    iPoseProof (own_update_frag_identity _ _ _ _ _ _ _ _ _ _ Hvalid_update with
    ...
    { iSplit; first (iPureIntro; exact Hvalid_old_kobj).
      iSplit; first (iPureIntro; exact Hupdated_old).
  ...
  { rewrite (kobject_updated_parent_ref _ _ Hupdated_new).
    exact Hinput_parent_ref_old. }
  assert ((KObjectV.objectmeta old_kobj).(ObjectMetaV.Namespace') =
  ...
  { exact Hlookup_abs. }
  { unfold new_kobj. rewrite KObjectV.status_update_objectmeta Hstatus_eq. done. }

...
Lemma wp_State__objCreate_without_name γ l kind namespace obj
  ptr kobj pure_kobj parent_key owned_parent owned_child_keys:
  {{{ is_pkg_init apimodel ∗
      ...
      "%Hpure_kobj_is_child" ∷ ⌜ obj_has_controller_parent_of pure_kobj parent_key.(KKey.Kind')
        parent_key.(KKey.Name') (PureKObject.objectmeta owned_parent).(PureObjectMeta.UID') ⌝ ∗
      "%Hwell_formed" ∷ ⌜ PureKObject.well_formed_for_nameless_create pure_kobj ⌝ ∗
  ...
  {{{ obj' ptr' kobj' pure_kobj', RET (#obj', #interface.nil);
      ...
      "Hdeepown_l'" ∷ PureKObject.deepown_l ptr' kobj' pure_kobj' 1 ∗
      "%Hwell_formed" ∷ ⌜ PureKObject.well_formed pure_kobj' ⌝ ∗
      "%Hnamespace_eq" ∷ ⌜ (PureKObject.objectmeta pure_kobj').(PureObjectMeta.Namespace') = namespace ⌝ ∗
      "%Hnew_key_notin" ∷ ⌜ (PureKObject.key pure_kobj') ∉ owned_child_keys ⌝ ∗
      "Hghost_pure_kobj'" ∷ (PureKObject.key pure_kobj') [[ γ.(γ_state) ]]↦ pure_kobj' ∗
      "Hown_parent" ∷ parent_key [[ γ.(γ_state) ]]↦ owned_parent ∗
      "Hown_child_keys" ∷  parent_key [[ γ.(γ_children) ]]↦ (owned_child_keys ∪ {[PureKObject.key pure_kobj']}) ∗
      "Hown_grandchild_keys" ∷ (PureKObject.key pure_kobj') [[ γ.(γ_children) ]]↦ ∅
      (* TODO: specify that pure_kobj' shares some content with pure_kobj *)
  }}}.
Proof.
  wp_start as "H". iNamed "H". iNamed "Hisk".
  pose proof PureKObject.well_formed_for_nameless_create_split _ Hwell_formed as [Hwell_formed_meta Hwell_formed_other].
  unfold PureObjectMeta.well_formed_for_nameless_create in Hwell_formed_meta.
  ...
  { unfold state_rep. unfold phys_state'. unfold abs_state'.
    ...
    - done. }
  assert (PureKObject.well_formed (PureKObject.update_objectmeta pure_kobj new_pure_objectmeta)) as Hwell_formed'.
  { assert (PureObjectMeta.well_formed new_pure_objectmeta) as Hwell_formed_meta'.
    { unfold PureObjectMeta.well_formed. 
      split_and!.
      1: intros; left; exists prefix; done. all: intuition. }
    pose proof PureKObject.well_formed_implies _ Hwell_formed_other as Hwell_formed_other'.
  ...
  assert (ghost_well_formed (dom used_uid') abs_state' children' fresh_keys ) as Hinv_Hghost_well_formed'.
  {
    ...
    assert (obj_has_controller_parent_of (PureKObject.update_objectmeta pure_kobj new_pure_objectmeta) (KKey.Kind' parent_key)
      (KKey.Name' parent_key) (PureObjectMeta.UID' (PureKObject.objectmeta owned_parent)))
      as Hpure_pod'_has_controller_parent_of_owned_parent.
    { destruct pure_kobj; done. }
    destruct Hinv_Hghost_well_formed.
    ...
    - intros key_p obj_p key_c obj_c s Hlookup_p Hlookup_c Hlookup_children.
      ...
      + intros Hobj_has_controller_parent_of.
        ...
        * pose proof (well_formed_obj_has_at_most_one_controller_parent
            (PureKObject.update_objectmeta pure_kobj new_pure_objectmeta) Hwell_formed' _ _ _ _ _ _
            Hobj_has_controller_parent_of Hpure_pod'_has_controller_parent_of_owned_parent)
    ...
    - intros k o kind name uid Hlookup_abs Hhas_parent.
      ...
      + pose proof (well_formed_obj_has_at_most_one_controller_parent
          (PureKObject.update_objectmeta pure_kobj new_pure_objectmeta) Hwell_formed' _ _ _ _ _ _ Hhas_parent
          Hpure_pod'_has_controller_parent_of_owned_parent) as (Hkind_eq & Hname_eq & ->).
        pose proof (Hexisting_uid_is_used parent_key owned_parent Hparent_key_in_abs).
...
Lemma wp_State__PodCreate_without_name γ l namespace ptr
  ...
  {{{ is_pkg_init apimodel ∗
      ...
      "%Hpure_pod_is_child" ∷ ⌜ obj_has_controller_parent_of (PureKObject.Pod pure_pod) parent_key.(KKey.Kind')
        parent_key.(KKey.Name') (PureKObject.objectmeta owned_parent).(PureObjectMeta.UID') ⌝ ∗
      "#Hwell_formed" ∷ ⌜ PurePod.well_formed_for_nameless_create pure_pod ⌝ ∗
      "Hown_parent" ∷ parent_key [[ γ.(γ_state) ]]↦ owned_parent ∗
      "Hown_child_keys" ∷ parent_key [[ γ.(γ_children) ]]↦ owned_child_keys
  }}}
    l @ (ptrT.id apimodel.State.id) @ "PodCreate" #namespace #ptr
  {{{ ptr' pod' pure_pod', RET (#ptr', #interface.nil);
      "Hdeepown_pod'" ∷ PurePod.deepown_l ptr' pod' pure_pod' 1 ∗
      ...
      "%Hnew_key_notin" ∷ ⌜ (PurePod.key pure_pod') ∉ owned_child_keys ⌝ ∗
      "Hown_pure_pod'" ∷ (PurePod.key pure_pod') [[ γ.(γ_state) ]]↦ (PureKObject.Pod pure_pod') ∗
      "Hown_parent" ∷ parent_key [[ γ.(γ_state) ]]↦ owned_parent ∗
      "Hown_child_keys" ∷  parent_key [[ γ.(γ_children) ]]↦ (owned_child_keys ∪ {[PurePod.key pure_pod']}) ∗
      "Hown_grandchild_keys" ∷ (PurePod.key pure_pod') [[ γ.(γ_children) ]]↦ ∅
...
Proof.
  ...
  with "[$Hdeepown_l $Hown_child_keys $Hown_parent]").
  { iFrame "#". done. }

...
From verification Require Import simplreplicaset_init.
...
Lemma wp_PodKey (pod: loc) (name namespace: go_string) :
  {{{ is_pkg_init simplreplicaset ∗
      "name" :: pod ↦s[simplreplicaset.Pod :: "Name"] name ∗
      "namespace" :: pod ↦s[simplreplicaset.Pod :: "Namespace"] namespace }}}
    @! simplreplicaset.PodKey #pod
  {{{ RET #(namespace ++ "/" ++ name)%go;
      pod ↦s[simplreplicaset.Pod :: "Name"] name ∗
  ...
  {{{ is_pkg_init simplreplicaset ∗
      ...
      "pods_els" :: ([∗ list] pod_l ∈ pods_els,
         ∃ (name namespace: go_string),
          "name" :: pod_l ↦s[simplreplicaset.Pod :: "Name"] name ∗
          "namespace" :: pod_l ↦s[simplreplicaset.Pod :: "Namespace"] namespace) }}}
    @! simplreplicaset.getPodKeys #pods
    {{{ (keys: slice.t) (keys_els: list go_string), RET #keys;
      ...
      ([∗ list] i ↦ pod_l; key ∈ pods_els; keys_els,
         ∃ (name namespace: go_string),
           pod_l ↦s[simplreplicaset.Pod :: "Name"] name ∗
           pod_l ↦s[simplreplicaset.Pod :: "Namespace"] namespace ∗
    ...
    iAssert (
      (([∗ list] pod_l ∈ drop (uint.nat (word.add i (W64 1))) pods_els,
        ∃ name namespace : go_string,
          pod_l ↦s[simplreplicaset.Pod :: "Name"] name ∗
          pod_l ↦s[simplreplicaset.Pod :: "Namespace"] namespace) ∗
        (∃ name : go_string, x ↦s[simplreplicaset.Pod :: "Name"] name) ∗
        (∃ namespace : go_string, x ↦s[simplreplicaset.Pod :: "Namespace"] namespace))%I
...
Lemma wp_getPodKeys_failed_with_induction (pods: slice.t) (pods_els: list loc) :
  {{{ is_pkg_init simplreplicaset ∗
      "pods" :: pods ↦* pods_els ∗
      "pods_els" :: ([∗ list] pod_l ∈ pods_els,
         ∃ (name namespace: go_string),
...
Proof.
  ...
  iIntros (keys) "(keys & own_keys_cap & %Heq)". wp_auto.
  iRename "pod" into "pod_ptr".
  wp_apply (wp_slice_for_range with "pods").
  iInduction pods_els as [|h t IH] forall (pods keys Hlen Heq).
  - simpl.
    iIntros "pods".
    wp_auto.
    iApply "HΦ".
    ...
    iSplit; done.
  - simpl. wp_auto.
    iDestruct "pods_els" as "((%name & %namespace & name & namespace) & pods_els)".
    wp_apply (wp_PodKey with "[$name $namespace]").
    iIntros "(name & namespace)".
    wp_auto.
    wp_apply (wp_slice_literal) as "%sl sl".
    wp_apply (wp_slice_append with "[$keys $own_keys_cap $sl]") as "%new_keys (new_keys & own_new_keys_cap & sl)".
    iSplit.
